...
const int BYPAOIN_IPC_CONNECT_TIMEOUT = 1000; // milliseconds
const QString BYPAOIN_IPC_PREFIX("ion:");
...
{
    QString name("YeePayssetsAssetsQt");
...
bool PaymentServer::ipcSendCommandLine()
{
    ...
    {
        if (!args[i].startsWith(BYPAOIN_IPC_PREFIX, Qt::CaseInsensitive))
            continue;
    ...
    foreach (const QString& arg, savedPaymentRequests)
    {
        ...
        if (!socket->waitForConnected(BYPAOIN_IPC_CONNECT_TIMEOUT))
            return false;
        ...
        socket->waitForBytesWritten(BYPAOIN_IPC_CONNECT_TIMEOUT);
