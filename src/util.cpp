...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\YeePayssetsAssets
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\YeePayssetsAssets
    // Mac: ~/Library/Application Support/YeePayssetsAssets
    // Unix: ~/.YeePayssetsAssets
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "YeePayssetsAsset";
#else
    fs::path pathRet;
    ...
    return pathRet / "YeePayssetsAsset";
#else
    // Unix
    return pathRet / ".YeePayssetsAsset";
#endif
...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "YeePayssetsAsset.conf"));
                    ...
                    map<string, vector<string> >& mapMultiSettingsRet)
{
    ...
    if (!streamConfig.good()){
        // Create empty YeePayssetsAssets.conf if it does not exist
        FILE* configFile = fopen(GetConfigFile().string().c_str(), "a");
		fputs("rpcuser=user \nrpcpassword=123456 \nserver=1 \nlisten=1 \nrpcallowip=127.0.0.1\nrpcport=27911", configFile);
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "YeePayssetsAssetsd.pid"));
