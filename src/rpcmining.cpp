...
Value checkkernel(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(-9, "YeePayssetsAssets is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(-10, "YeePayssetsAssets is downloading blocks...");
...
Value getwork(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "YeePayssetsAssets is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "YeePayssetsAssets is downloading blocks...");
